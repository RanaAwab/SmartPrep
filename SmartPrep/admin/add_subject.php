...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name) || empty($code) || empty($course_id)) {
        $error = "All fields are required!";
    } elseif (strlen($code) < 2) {
        $error = "Subject code too short!";
    } else {
        // Check duplicate code
        $existing = fetch("SELECT id FROM subjects WHERE code = ?", [$code]);
...
<a href="manage_subjects.php" class="page-top-link">
    <i class="bi bi-arrow-left"></i> Back to Subjects
</a>

<div class="form-card">
    <h2 class="dash-title">Add Subject</h2>
    ...
    <form method="POST">
        <div class="mb-4">
            ...
            <input type="text" name="name" class="form-control-custom" placeholder="e.g. Data Structures" required>
        </div>

        <div class="mb-4">
            <label class="form-label-custom">Subject Code</label>
            <input type="text" name="code" class="form-control-custom" placeholder="e.g. CS201" required>
        </div>

        <div class="mb-4">
            <label class="form-label-custom">Associated Course</label>
        ...
        <div class="text-center mt-2">
            <button class="btn-save w-100">Create Subject</button>
        </div>
    </form>
</div>
