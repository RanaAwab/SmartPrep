...
// Fetch subject
$subject = fetch("SELECT * FROM subjects WHERE id = ?", [$id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name) || empty($code) || empty($course_id)) {
        ...
        if ($existing) {
            ...
            executeQuery(
                "UPDATE subjects SET name = ?, code = ?, course_id = ? WHERE id = ?",
                [$name, $code, $course_id, $id]
            );
...
<div class="form-card">
    <h2 class="dash-title">Edit Subject</h2>
    ...
    <form method="POST">
        ...
        <div class="text-center mt-2">
            <button class="btn-save w-100">Update Subject</button>
        </div>
    </form>
</div>
