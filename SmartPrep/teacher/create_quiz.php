...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($subject_id) || empty($title)) {
        ...
        $insert = executeQuery(
            "INSERT INTO quizzes (subject_id, teacher_id, title)
             VALUES (?, ?, ?)",
            [$subject_id, $teacher_id, $title]
        );

        if ($insert) {
            $success = "Quiz created! You can now add questions to it.";
        } else {
...
<a href="add_mcq.php" class="page-top-link float-end mt-2">
    Step 2: Add MCQs <i class="bi bi-arrow-right"></i>
</a>

<div class="form-card mt-3">
    <h2 class="dash-title">Create New Quiz</h2>
    ...
    <form method="POST">
        ...
        <div class="mb-4">
            <label class="form-label-custom">Quiz Title</label>
            <input type="text" name="title" class="form-control-custom" placeholder="e.g. Midterm 1 - Networking" required>
        </div>

        <div class="text-center mt-2">
            <button class="btn-save w-100">Create Quiz Framework</button>
        </div>
    </form>
</div>
